...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
    ...
    static final String TAG = "com.gesoftware.services.GameHelper";
    ...
    /**
     * Construct a com.gesoftware.services.GameHelper object, initially tied to the given Activity.
    ...
    void assertConfigured(String operation) {
        if (!mSetupDone) {
            String error = "com.gesoftware.services.GameHelper error: Operation attempted without setup: "
                    + operation
    ...
    private void doApiOptionsPreCheck() {
        if (mGoogleApiClientBuilder != null) {
            String error = "com.gesoftware.services.GameHelper: you cannot call set*ApiOptions after the client "
    ...
    public GoogleApiClient.Builder createApiClientBuilder() {
        if (mSetupDone) {
            String error = "com.gesoftware.services.GameHelper: you called com.gesoftware.services.GameHelper.createApiClientBuilder() after "
    ...
    /**
     * Performs setup on this com.gesoftware.services.GameHelper object. Call this from the onCreate()
    ...
    public void setup(GameHelperListener listener) {
        if (mSetupDone) {
            String error = "com.gesoftware.services.GameHelper: you cannot call com.gesoftware.services.GameHelper.setup() more than once!";
            logError(error);
            throw new IllegalStateException(error);
        }
        mListener = listener;
    ...
    public void onStart(Activity act) {
        ...
        if (mConnectOnStart) {
            if (mGoogleApiClient.isConnected()) {
                Log.w(TAG,
                        "com.gesoftware.services.GameHelper: client was already connected on onStart()");
            } else {
    ...
    public void enableDebugLog(boolean enabled, String tag) {
        Log.w(TAG, "com.gesoftware.services.GameHelper.enableDebugLog(boolean,String) is deprecated. "
                + "Use com.gesoftware.services.GameHelper.enableDebugLog(boolean)");
    ...
    public static void showFailureDialog(Activity activity, int actResp,
                                         int errorCode) {
        if (activity == null) {
            Log.e("com.gesoftware.services.GameHelper", "*** No Activity. Can't show failure dialog!");
            return;
        }
        ...
        switch (actResp) {
            ...
            default:
                ...
                if (errorDialog == null) {
                    // get fallback dialog
                    Log.e("com.gesoftware.services.GameHelper",
    ...
    void debugLog(String message) {
        if (mDebugLog) {
            Log.d(TAG, "com.gesoftware.services.GameHelper: " + message);
        }
    }

    void logWarn(String message) {
        Log.w(TAG, "!!! com.gesoftware.services.GameHelper WARNING: " + message);
    }

    void logError(String message) {
        Log.e(TAG, "*** com.gesoftware.services.GameHelper ERROR: " + message);
    }
    ...
    // to a given state. This may be useful when using com.gesoftware.services.GameHelper in a
    // non-standard
    // sign-in flow.
